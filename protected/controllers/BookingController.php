...
class BookingController extends Controller
{
    ...
    public function revertDateFormat($date) {
	...
	public function actionSave(){
		...
		if(isset($_POST['preview'])){
			...
			$customer->createdOn = $this->convertDateFormat($_POST['createdOn']);;
			$customer->dob = $this->convertDateFormat($_POST['dob']);;
		...
		if($_POST['plot_id']){
			...
			if(!$CP){
				...
				if($customer->save(false)){
					...
					$customer_plot->attributes = $_POST;
					$customer_plot->agent_cnic = '';
					$customer_plot->id = $_POST['id'];
					//if($plot->agentReserve){
						$customer_plot->is_agent = 1;
					...
					//}
					$customer_plot->monthlyYearlies = $_POST['monthlyMonths'];
					$customer_plot->charge_id = 1;
					$customer_plot->user_id = Yii::app()->session['userModel']['id'];
					$customer_plot->customer_id = $customer->id;
					//$customer_plot->createdOn =  @$_POST['createdOn'];
					$customer_plot->createdOn = $this->convertDateFormat($_POST['createdOn']);;
			        $customer_plot->monthly_start_date = $this->convertDateFormat($_POST['monthly_start_date']);;
					$customer_plot->updatedBy = Yii::app()->session['userModel']['first_name'].' '.Yii::app()->session['userModel']['last_name'];
					$customer_plot->status = 1;
					$customer_plot->is_special = $_POST['is_special'];
					$customer_plot->blocked = 0;
	...
	public function actionAddoldtransaction($id){
		$phaseId = Yii::app()->session->get('userModel')['phase_id'];
		$booking = CustomerPlots::model()->findByPk($id);
		$sizeId = $booking->plot->size->id;
		if($booking){
		...
		$this->render('transactionOld',$data);
	}
...
// 			}
	...
	{
		$phaseId = Yii::app()->session->get('userModel')['phase_id'];
		//$data['paymentSchedules'] = Plot::model()->findAll();
		...
		if(array_key_exists('block', $_GET)){
			$s = $_GET['block'];
			$data['block'] = $_GET['block'];
			$criteria->addCondition("plot.block_number = '$s'");
		}
	...
	public function actionEditbooking($id)
	{
		...
		//$data['plots'] = Plots::model()->findAll('status = 0 AND id = {$booking->plot_id}');
		$data['paymentmodes'] = PaymentModes::model()->findAll('amount = 0');
		$data['booking'] = $booking =  CustomerPlots::model()->findByPk($id);
		$criteria = new CDbCriteria();
        $criteria->condition = 'status = 0 OR id = :id';
        $criteria->params = array(':id' => $booking->plot_id);
        $data['plots'] = Plots::model()->findAll($criteria);
	...
	public function actionUpdate(){
		...
		if($_POST['plot_id']){
			...
			$customer->status = 1;
			$customer->dob = $this->convertDateFormat($_POST['dob']);;
			//$customer->createdOn =  $this->convertDateFormat($customer->createdOn.' '.date('H:i:s'));
			unset($customer->createdOn);
			$customer->save(false);
			if($customer){
			    ...
			    $customer_plot->status = 1;
				$customer_plot->customer_id = $customer->id;
				if($_POST['plot_id'] != $_POST['updated_plot_id']){
    			    $customer_plot->plot->status = 0;
    			    $customer_plot->plot->save(false);
    			    $customer_plot->plot_id = $_POST['updated_plot_id'];
    			}
    			if($_POST['agent_id']){
				    $agentDet = Agents::model()->findByPk($_POST['agent_id']);
				    $customer_plot->agent_percentage = $agentDet->percentage_value;
                ...
                if($_POST['plot_id'] != $_POST['updated_plot_id']){
                    $plot = Plots::model()->findByPk($_POST['updated_plot_id']);
                    $plot->status = 1;
                    $plot->save(false);
                }
				...
				if(isset($_POST['mode_id'])){
					foreach($_POST['mode_id'] as $ind => $modes){
					    ...
					    // If mode_id is a string, save it as an extra transaction
                        if (is_string($modes) && !is_numeric($modes) && !empty($modes)) {
                            $plotOld = CustomerPlotTransactions::model()->findByPk($_POST['transaction_id'][$ind]);
                            $plotOld->updatedBy = Yii::app()->session['userModel']['first_name']
                                    . ' ' .
                                    Yii::app()->session['userModel']['last_name'].'-'.date('Y-m-d');
                        	$plotOld->reason = 'Updated to Extra';
							$plotOld->status = 0;
							$plotOld->save(false);
                            if (@$_POST['t_reason'][$ind] == '') {
                                ...
                                $plot->createdOn = $this->convertDateFormat(
                                    @$_POST['t_date'][$ind]
                                );
                                $plot->phase_id = Yii::app()->session->get('userModel')['phase_id'];
                                $plot->createdBy = Yii::app()->session['userModel']['first_name']
                                    . ' ' .
                                    Yii::app()->session['userModel']['last_name'];
                                $plot->updatedBy = Yii::app()->session['userModel']['first_name']
                                    . ' ' .
                                    Yii::app()->session['userModel']['last_name'];
                                $plot->monthlyDate = Yii::app()->session['userModel']['id']
                                    . '--' . date('Y-m-d H:i');
                                $plot->status = 1;
                            ...
                            // Don't execute the normal mode_id logic
                            continue;
                        }
						...
						if($plot){
							if($_POST['t_reason'][$ind] == ''){
								...
								$plot->comment = @$_POST['t_comment'][$ind];
								$plot->reference_number = @$_POST['t_reference_number'][$ind];
								$plot->createdOn =  $this->convertDateFormat(@$_POST['t_date'][$ind]);
								$plot->phase_id = Yii::app()->session->get('userModel')['phase_id'];
								if(isset($_POST['t_monthlyDate'][$plot->id]) && !empty($_POST['t_monthlyDate'][$plot->id])){
    							    $plot->monthlyDate = (@implode(',',@$_POST['t_monthlyDate'][$plot->id])).'*'.Yii::app()->session['userModel']['id'].'--'.date('Y-m-d H:i');    
    							} else{
    							    $plot->monthlyDate = Yii::app()->session['userModel']['id'].'--'.date('Y-m-d H:i');
    							}
								$plot->updatedBy = Yii::app()->session['userModel']['first_name'].' '.Yii::app()->session['userModel']['last_name'];
								$plot->status = 1;
								$plot->save(false);	
						...
						if(!empty($modes)){
							...
							if($plot){
								if($_POST['extra_t_reason'][$ind] == ''){
									...
									$plot->createdOn =  $this->convertDateFormat(@$_POST['extra_t_date'][$ind]);
									$plot->phase_id = Yii::app()->session->get('userModel')['phase_id'];
									$plot->updatedBy = Yii::app()->session['userModel']['first_name'].' '.Yii::app()->session['userModel']['last_name'];
									$plot->monthlyDate = Yii::app()->session['userModel']['id'].'--'.date('Y-m-d H:i');
									$plot->status = 1;
									...
									$plot->reason = @$_POST['extra_t_reason'][$ind];
									$plot->status = 0;
									$plot->save(false);
								}
							}
	...
	public function plotTotal($id,$number_format = true,$is_total = true,$withside = true){
        ...
        if($plot->is_road_facing == 1){
            $roadCharger = $this->Percentage($plot->total,$plot->is_road_facing_amount,0);
            $plotTotal = $plotTotal + $roadCharger;
        }
        
        if($plot->is_corner == 1){
            $cornerCharger = $this->Percentage($plot->total,$plot->is_corner_amount,0);
            $plotTotal = $plotTotal + $cornerCharger;
        }

        if($plot->is_park_facing == 1){
            $parkFacing = $this->Percentage($plot->total,$plot->is_park_facing_amount,0);
            $plotTotal = $plotTotal + $parkFacing;
        }

        if($plot->is_west_open == 1){
            $westOpen = $this->Percentage($plot->total,$plot->is_west_open_amount,0);
	...
	public function actionprintinvoice($id,$last=''){
		...
		//$transactionExtra = CustomerPlotExtraTransactions::model()->findAll("id >= :lastId AND plot_id = $id",array(':lastId'=>$last));
		$transactionExtra = CustomerPlotExtraTransactions::model()->findAll("plot_id = $plot$plottransaction_number LIKE :key",array(':key'=>"%".$transaction."%"));
	...
	public function actionSavetransaction(){
		...
		if(isset($_POST['mode'])){
				...
				try {
				    $connection->createCommand("INSERT INTO transaction_sequence VALUES ()")->execute();
                    $transactionNumberUpdated = $connection->getLastInsertID();
                    
				    //Insert dummy row to generate unique number
                    // if($userModel['id'] == 29){
                    //     $transactionNumberUpdated = $_POST['transaction'][0];
                    // } else{
                    //     $connection->createCommand("INSERT INTO transaction_sequence VALUES ()")->execute();
                    //     $transactionNumberUpdated = $connection->getLastInsertID();
                    // }
    				...
    				foreach($_POST['mode'] as $ind => $modes){
    						...
    						if(is_numeric($modes)){
    							...
    							$plot->plot_payment_mode_id = $modes;
    							$plot->transaction_number = $transactionNumberUpdated;//$_POST['transaction'][$ind];
    							$plot->amount = $_POST['amount'][$ind];
    							...
    							$plot->createdBy = Yii::app()->session['userModel']['first_name'].' '.Yii::app()->session['userModel']['last_name'];//@$_POST['createdBy'];//Yii::app()->session['userModel']['first_name'].' '.Yii::app()->session['userModel']['last_name'];
    							$plot->updatedBy = Yii::app()->session['userModel']['first_name'].' '.Yii::app()->session['userModel']['last_name'];//@$_POST['createdBy'];//Yii::app()->session['userModel']['first_name'].' '.Yii::app()->session['userModel']['last_name'];
    							if(!empty($_POST['monthlyDate'])){
    							    $plot->monthlyDate = (@implode(',',@$_POST['monthlyDate'])).'*'.Yii::app()->session['userModel']['id'].'--'.date('Y-m-d H:i');    
    							} else{
    							    $plot->monthlyDate = Yii::app()->session['userModel']['id'].'--'.date('Y-m-d H:i');
    							}
    							$plot->status = 1;
				...
				if($cplots->customerPlotCancelled){
				    $cancelledBooking  = CustomerPlotCancelled::model()->find('booking_id = :booking',array(':booking'=>$_POST['plot_id']));
    		        if($cancelledBooking){
				...
				$total = array_sum(@$_POST['amount']);
                $cplots = CustomerPlots::model()->findByPk($_POST['plot_id']);
                $customerName = trim($cplots->customer->name);
                $firstName = strtok($customerName, ' ');
				//$msg = "Dear ".ucfirst($cplots->customer->name).",\nThankyou for Paying Amount: ".number_format($total)."/= against your Plot ".$cplots->plot->block_number.'-'.$cplots->plot->plot_number.".\n\nGulshan-e-Bholari";
				$msg = "Dear ".ucfirst($firstName)." (".$this->getBookingRegNo($cplots->id)."),\nWe have received your payment of  Rs. ".number_format($total)."/= against Plot # ".$cplots->plot->block_number.'-'.$cplots->plot->plot_number.".\nGulshan-e-Bholari Housing Scheme";
				//$msg = "Dear ".ucfirst($firstName).",\nWe have received your payment of  Rs. ".$total."/= against File # ".$this->getBookingRegNo($cplots->id).".\nFor detail, please call 0213-4960000\n\nGulshan-e-Bholari Housing Scheme";
		...
		$l = Yii::app()->baseUrl.'/booking/dublicateinvoice/plot/'.$_POST['plot_id'].'/transaction/'.$transactionNumberUpdated;//$_POST['transaction'][$ind];
		//$l = Yii::app()->baseUrl.'/booking/dublicateinvoice/plot/'.$_POST['plot_id'].'/transaction/'.$_POST['lastTransactionIdNew'];
	...
	public function actionSaveoldtransaction(){
				...
				try {
				    foreach($_POST['mode'] as $ind => $modes){
    						if(is_numeric($modes)){
    							...
    							$plot->plot_id = @$_POST['plot_id'];
    							$plot->plot_payment_mode_id = $modes;
    							$plot->transaction_number = $_POST['transaction'][0];
    							$plot->amount = $_POST['amount'][$ind];
    							$plot->monthlyDate = @$_POST['monthlyDate'];//@$_POST['monthlyDate'][$ind];
    							$plot->transaction_type = @$_POST['transaction_type'][$ind];
    							$plot->reference_number = $_POST['reference_number'][$ind];
    							$plot->comment =  @$_POST['comment'];
    							$plot->bank =  @$_POST['bank'][$ind];
		...
		Yii::app()->user->setFlash('success','Plot old transaction added successfully.');
        $this->redirect(Yii::app()->baseUrl.'/booking/viewbooking/'.$_POST['plot_id']);
	}
	...
	public function actionGetModeDueDate($booking,$date,$mode,$checkValue=false){
		...
		switch ($mode) {
			...
			case 'demarcation':
				...
				break;
			case 'possession':
				return date('d M, Y',strtotime(date("Y-m-d", strtotime($date)) . "+36 months"));
				break;
	...
	public function actionCancel($id){
		...
		$plotBookingExtraSum = $booking->customerPlotExtraTransactionSum;
        ...
        $tpp = $booking->plot->total - $booking->plot->discount;
        ...
        if ($booking->plot->is_corner == 1) {
            $amount = $this->Percentage($booking->plot->total, $booking->plot->is_corner_amount, false);
            $extraTotal += $amount;
        }
        
        // Road Facing
        if ($booking->plot->is_road_facing == 1) {
            $amount = $this->Percentage($booking->plot->total, $booking->plot->is_road_facing_amount, false);
            $extraTotal += $amount;
        }
        
        // Park Facing
        if ($booking->plot->is_park_facing == 1) {
            $amount = $this->Percentage($booking->plot->total, $booking->plot->is_park_facing_amount, false);
            $extraTotal += $amount;
        }
        
        // West Open
        if ($booking->plot->is_west_open == 1) {
            $amount = $this->Percentage($booking->plot->total, $booking->plot->is_west_open_amount, false);
            $extraTotal += $amount;
        }
        
        // Final total including extras
        $data['total'] = $tpp + $extraTotal;
	...
	public function actionCancelsubmit(){
		...
		if(!$cancelledBooking){
			...
			$model->createdOn = $this->convertDateFormat($_POST['cancel_date']);
			$model->save(false);
	...
	public function actionTransfersubmit(){
		...
		if($booking){
			...
			if($customer->save(false)){
				...
				if($booking->customerpaymentSchedule){
					foreach ($booking->customerpaymentSchedule as $key => $value) {
						$customerCustPaySche = New CustomPaymentSchedulePaymentModes;
						$customerCustPaySche->attributes = $value->attributes;
						$customerCustPaySche->booking_id = $customer_plot->id;
						$customerCustPaySche->save(false);
					}
				}
	...
	public function getPlotLedgerDetail($id){
		...
		if($booking->customerpaymentSchedule){
		    ...
		    $netTotalCheck = $booking->customPaymentScheduleTotal() - intval($booking->customerPlotTransactionSum);    
		}
		...
		foreach($paymentmodes as $modes): $rowBalance = 0;
			...
			if($booking->customerpaymentSchedule){
				$cpt = CustomerPlotTransactions::model()->findAll('status = 1 AND plot_id = :id AND plot_payment_mode_id = :modeid',array(':id'=>$booking->id,':modeid'=>@$modeIds[$modes->mode]));
				$var_sum = CustomerPlotTransactions::model()->findBySql('select sum(`amount`) as `total` from customer_plot_transactions WHERE status = 1 AND plot_id = :id AND plot_payment_mode_id = :modeid', array(':id'=>$booking->id,':modeid'=>@$modeIds[$modes->mode]));
			} else{
				$cpt = CustomerPlotTransactions::model()->findAll('status = 1 AND plot_id = :id AND plot_payment_mode_id = :modeid',array(':id'=>$booking->id,':modeid'=>$modes->id));
				$var_sum = CustomerPlotTransactions::model()->findBySql('select sum(`amount`) as `total` from customer_plot_transactions WHERE status = 1 AND plot_id = :id AND plot_payment_mode_id = :modeid', array(':id'=>$booking->id,':modeid'=>$modes->id));
			}
			...
			if($modes->mode=='possession'){
				if($modes->amount != $var_sum->total){
			    	...
			    	//$html .= '<b>'.$this->actionGetModeDueDate(@$booking, @$booking->createdOn,strtolower(@$modes->mode),round($checkValue)).'-->'.$modes->mode.'---->'.floor($divideVal).'</b></br/>';
			    	//break;
			    	$dddd = $this->actionGetModeDueDate(@$booking, @$booking->monthly_start_date,strtolower(@$modes->mode),round($checkValue));
				    $ddddY = explode(',',$dddd);
			    	$ddddM = date('m',strtotime($dddd));
			    	if(strtotime(date('Y-m')) >= strtotime(date(trim($ddddY[1]).'-'.$ddddM))){
			    		$html .= '<p>Possession &nbsp;<b>Total: '.number_format(floor($divideVal),'2','.',',').'</b></p>';
						$finalTotal += number_format(floor($divideVal),'2','.','');
			    	}
					//break;
				}
		...
		//for monthly & yearly
		$dd = $this->actionLedgerOnly($id);
		if(@$dd['monthly'] > 0){
			$html .= '<p><b>'.$dd['monthlyDue'].'</b> Monthly &nbsp;<b>Total: '.number_format($dd['monthly'],'2','.',',').'</b></p>';	
			$finalTotal += $dd['monthly'];
			$monthlyCount = $dd['monthlyDue'];
		}

		if(@$dd['yearly'] > 0){
			$html .= '<p><b>'.$dd['yearlyDue'].'</b> Yearly &nbsp;<b>Total: '.number_format($dd['yearly'],'2','.',',').'</b></p>';
			$finalTotal += $dd['yearly'];
		}
	...
	public function getPlotLedgerDetailSingle($id,$mode,$csv=false,$invoiceTransactionId = NULL){
		...
		foreach($paymentmodes as $modes): $rowBalance = 0;
			...
			if($modes->mode=='monthly' && $modes->mode == $mode){
				if($booking->customerpaymentSchedule){
					if($invoiceTransactionId){
						$var_sum = CustomerPlotTransactions::model()->findBySql('select sum(`amount`) as `total` from customer_plot_transactions WHERE status = 1 AND plot_id = :id AND plot_payment_mode_id = :modeid AND id <= :invoice', array('invoice'=>$invoiceTransactionId,':id'=>$booking->id,':modeid'=>$modeIds[$mode]));
					} else{
						$var_sum = CustomerPlotTransactions::model()->findBySql('select sum(`amount`) as `total` from customer_plot_transactions WHERE status = 1 AND plot_id = :id AND plot_payment_mode_id = :modeid', array(':id'=>$booking->id,':modeid'=>$modeIds[$mode]));
					}
				} else{
					if($invoiceTransactionId){
			    ...
			    if($var_sum->total >= $modes->amount){
			    	$checkValuePaid = $checkValue-1;
			    	$html .= ' 36 out of 36 Monthly installments, <b>Paid Upto: '.date('M, Y',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "+36 months")).'</b><br/>';
			    ...
			    if($checkValue != 36 && $isDetailShowMonthly == 1){
                	if($netTotalCheck > 0){
	                        if($checkValue!=0){
	                            $checkValuePaid = $checkValue-1;
	                            $html .= $checkValue.' out of 36 Monthly installments, <b>Paid Upto: '.date('M, Y',strtotime(date("Y-m", strtotime($booking->monthly_start_date)) . "+$checkValuePaid months")).'</b><br/>';
	                        } else{
	                        	$checkValuePaid = $checkValue-1;
	                        	$html .= $checkValue.' out of 36 Monthly installments, <b>Paid Upto: '.date('M, Y',strtotime(date("Y-m", strtotime($booking->monthly_start_date)) . "+$checkValuePaid+1 months")).'</b><br/>';
	                        }
	                        if($csv){
	                        	$html = $checkValue;
	                        }
                    	...
                    	$checkValuePaid = $checkValue-1;
                        	$html .= $checkValue.' out of 3655 Monthly installments, <b>Paid Upto: '.date('M, Y',strtotime(date("Y-m", strtotime($booking->monthly_start_date)) . "+$checkValuePaid+1 months")).'</b><br/>';
                    }
                    break;
                }
			}
			if($modes->mode=='yearly' && $modes->mode == $mode){
			    ...
			    if($var_sum->total >= $modes->amount){
			    	$checkValuePaid = 36;
			    	$html .= ' 6 out of 6 Half Yearly installments, <b>Paid Upto: '.date('M, Y',strtotime(date("Y-m-d", strtotime($booking->monthly_start_date)) . "+36 months")).'</b><br/>';
	...
	public function actionGetLedgerDetail(){
		...
		foreach($cp as $c){
			...
			if(isset($_GET['remaining'])){
				...
				$mce = explode('-',$_GET['monthlyCount']);
				if($det['monthlyCount'] >= $mce[0] && $det['monthlyCount'] <= $mce[1] && $det['amount'] > 0){
						$result[$c->id]['ledger'] = @$det['html'];
						$result[$c->id]['amount'] = @$det['amount'];	
						$result[$c->id]['monthlyCount'] = @$det['monthlyCount'];		
				} else{
	...
	*/
	...
	public function actionFetchall(){
		...
		if($flagStatus != 'false'){
			$criteria->addCondition("t.flag_status = $flagStatus");
		}

		if($type != 'false'){
			$criteria->addCondition("plot.plot_type = '$type'");
		}
		...
		if(!empty(@$result_array['search']['value'])){
			...
			if($searchType == 'regNo'){
				$query = str_replace('*','',$result_array['search']['value']);
				//$queryExplode = explode('-',$query);
				
				// if(array_key_exists(0, $queryExplode)){
				// 	$criteria->addCondition('plot.block_number = :block');	
				// 	$params[':block'] = @$queryExplode[0];
				// }
				// if(array_key_exists(1, $queryExplode)){
				// 	$criteria->addCondition('plot.plot_type = :type');
				// 	$params[':type'] = @$queryExplode[1];
				// }
				//if(array_key_exists(2, $queryExplode)){
					$criteria->addCondition('t.id = :plot');
					$params[':plot'] = @$query;
				//}
			...
			if($searchType == 'plot'){
				$query = str_replace('*', '', $result_array['search']['value']);
                $queryExplode = explode('-', $query);
                //print_r($queryExplode);exit;
        		$plotType=@$queryExplode[0];
        		$plotNumber=@$queryExplode[1];
        		$blockNumber=@$queryExplode[2];
                //if(count($parts) >= 2){
                    // Always apply block if exists
                    if($blockNumber != ''){
                        $criteria->addCondition('plot.block_number = :blockNumber');
			...
			if($searchType == 'name'){
				$criteria->addCondition("customer.name LIKE '%".$result_array['search']['value']."%'");
			}

			if($searchType == 'cnic'){
				$criteria->addCondition("customer.cnic LIKE '%".$result_array['search']['value']."%'");
			}

			if($searchType == 'number'){
				$criteria->addCondition("customer.mobile LIKE '%".$result_array['search']['value']."%'");
			}
		}
		
		//echo '<pre>';print_r($criteria);exit;
		$model = CustomerPlots::model()->with('plot','customer')->findAll($criteria);
		
		$result = array();
		$i=0;
		$countTotal = CustomerPlots::model()->with('plot','customer')->count($criteria);
		...
		if($model){
			foreach($model as $booking):
				...
				$cpText = '';
				if(@$booking->customerpaymentSchedule){
					$cpText = '<span class="aLink label label-info">CP</span>';
				}
				$result['data'][$i][] = $this->getBookingRegNo($booking->id).'<br/>'.@$cpText;
                $result['data'][$i][] = $booking->customer->name;
                $result['data'][$i][] = $booking->customer->cnic;
                $result['data'][$i][] = $booking->customer->mobile;

                $status = '';
                $status .= '<span class="label label-success" style="text-decoration: none;">Booked</span>';
                if($booking->blocked != 1 && $booking->blocked != 2){
                    if(empty($booking->customerPlotCancelled)){
                    	//$status .= ($booking->status==1)?(($complete==0)?'<span class="aLink label label-primary">Booked '.(($trasferred==1)?'(Transferred)':'').'</span>':'<span class="aLink label label-success">Completed</span>'):'<span class="aLink label label-danger">Temporary Booked</span>';
	                        
	                        if($booking->customerPlotTransactionSum >= $this->discountedPlotCostOfLand($booking->plot->id)) {
	                    		//$status .= '<br/><span class="label label-warning" style="text-decoration: none;">Cost of Land Paid</span>';
	                       	}

	                    	if($booking->customerPlotTransactionSum >= $this->discountedPlotCostOfLandAndExtra($booking->plot->id)) {
	                    		//$status .= '<br/><span class="label label-success" style="text-decoration: none;">Plot Total Paid</span>';
	                    	}

	                    	//$status .= '<br/>'.@$documentCompleted;
                    } else{ 
                        $status = '<span class="aLink label label-danger">Cancelled</span>';
                    }
                } else {
                    $status = '<span class="aLink label label-danger">Blocked</span>';
                }
                $result['data'][$i][] = $status;


                $button = '<a href="'.Yii::app()->baseUrl.'/booking/viewbooking/'.$booking->id.'"><span class="aLink label label-info">View</span></a>&nbsp;';
                if($userModel['user_type']['id'] == 1){
                	if($booking->flag_status==2){
                    	//$button .= '<span class="aLink label label-success">Received By Customer</span>&nbsp;';
               		} else if($booking->flag_status==1){
                    	//$button .= '<a href="'.Yii::app()->baseUrl.'/api/setbookingflag/view/0/flag/2/booking/'.$booking->id.'"><span class="flagLink label label-primary">Rec By Customer?</span></a>&nbsp;';
                	} else{
                    	//$button .= '<a href="'.Yii::app()->baseUrl.'/api/setbookingflag/view/0/flag/1/booking/'.$booking->id.'"><span class="flagLink label label-danger">File Complete?</span></a>&nbsp;';
                	}
            	}
                if(empty($booking->customerPlotCancelled)){
                        if($userModel['user_type']['id'] == 1){
                            $button .= '<a href="'.Yii::app()->baseUrl.'/booking/editbooking/'.$booking->id.'"><span class="aLink label label-info">Edit</span></a>&nbsp;';
                            $button .= '<a href="'.Yii::app()->baseUrl.'/booking/addtransaction/'.$booking->id.'"><span class="aLink label label-success">Add Transaction</span></a>';
                        }
               	}

                $result['data'][$i][] = $button;
                $result['data'][$i][] = 'Rs. '.$this->plotTotal($booking->plot->id).' / Rs. '.number_format($netTotal);
                $result['data'][$i][] = 'Rs. '.$this->plotTotalDiscount($booking->plot->id);
                $result['data'][$i][] = ($booking->agent_id)?@$booking->agent->name:'-';
				$i++;
			endforeach;	
		}
	...
	public function getPlotLedgerDetailCustom($id){
		...
		$netTotalCheck = $this->plotDiscount($booking->plot->id,false) - intval($booking->customerPlotTransactionSum);
		if($booking->customerpaymentSchedule){
			$origModes = PaymentSchedulePaymentModes::model()->findAll('payment_schedule_id = :id',array(':id'=>1));
			$modeIds = [];
			//echo '<pre>';print_r($origModes);exit;
			array_map(function($item) use(&$modeIds) {
				$modeIds[strtolower($item['mode'])] = $item['id'];
			},$origModes);
			
		    $paymentmodes = CustomPaymentSchedulePaymentModes::model()->findAll('booking_id = :id',array(':id'=>$booking->id));
		    $netTotalCheck = $booking->customPaymentScheduleTotal() - intval($booking->customerPlotTransactionSum);    
		    //echo '<pre>';print_r($modeIds);print_r($paymentmodes);exit;
		}
		...
		$final = [];
		foreach($paymentmodes as $modes): $rowBalance = 0;
			//echo '<pre>';print_r($modes);print_r($modeIds[$modes->mode]);exit;
			//$cpt = CustomerPlotTransactions::model()->findAll('status = 1 AND plot_id = :id AND plot_payment_mode_id = :modeid',array(':id'=>$booking->id,':modeid'=>$modes->id));
			//$var_sum = CustomerPlotTransactions::model()->findBySql('select sum(`amount`) as `total` from customer_plot_transactions WHERE status = 1 AND plot_id = :id AND plot_payment_mode_id = :modeid', array(':id'=>$booking->id,':modeid'=>$modes->id));
			if($booking->customerpaymentSchedule){
				$cpt = CustomerPlotTransactions::model()->findAll('status = 1 AND plot_id = :id AND plot_payment_mode_id = :modeid',array(':id'=>$booking->id,':modeid'=>@$modeIds[$modes->mode]));
	...
	public function actionLedgerOnly($id){
		//header('Content-Type: application/json; charset=utf-8');
		$data['booking'] = $booking = CustomerPlots::model()->findByPk($id);
		if($booking->customerpaymentSchedule){
			$d = $this->renderPartial('ledgerOnly-custom',$data, true);
		} else{
			$d = $this->renderPartial('ledgerOnly',$data, true);
		}
		
		preg_match('~\{(?:[^{}]|(?R))*\}~', $d, $json);
		if($json && array_key_exists(0, $json)){
			$JsonResults = print_r($json[0], true);
	...
	public function actionDeletetransaction($id,$type){
	    if($type=='other'){
	        $cpet = CustomerPlotExtraTransactions::model()->findByPk($id);
    	    if($cpet){
    	        $cpet->delete();
    	    }
	    } else{
	        $cpt = CustomerPlotTransactions::model()->findByPk($id);
    	    if($cpt){
    	        $cpt->delete();
    	    }    
	    }
        ...
        $this->redirect(Yii::app()->baseUrl.'/transactions');
