...
// 			}
	...
	public function actionSavetransaction(){
		...
		if(isset($_POST['mode'])){
				...
				foreach($_POST['mode'] as $ind => $modes){
						...
						if(!in_array($modes,$extraTransMode)){
							...
							$plot->createdBy = @$_POST['createdBy'];//Yii::app()->session['userModel']['first_name'].' '.Yii::app()->session['userModel']['last_name'];
							$plot->updatedBy = @$_POST['createdBy'];//Yii::app()->session['userModel']['first_name'].' '.Yii::app()->session['userModel']['last_name'];
							$plot->status = 1;
							$plot->save(false);
		...
		$l = Yii::app()->baseUrl.'/booking/dublicateinvoice/plot/'.$_POST['plot_id'].'/transaction/'.$_POST['transaction'][$ind];
	...
	*/
	...
	public function actionFetchall(){
		...
		if($payment != 'false'){
			$criteria->addCondition("t.is_special = $payment");
		}
		...
		if(!empty(@$result_array['search']['value'])){
			...
			if($searchType == 'plot'){
				$criteria->addCondition("plot.plot_number LIKE '%".$result_array['search']['value']."%'");
			}
		...
		if($model){
			foreach($model as $booking):
				...
				$result['data'][$i][]= '<a href="'.Yii::app()->baseUrl.'/booking/viewbooking/'.$booking->id.'">'.('*'.$booking->plot->block_number.'-'.$booking->plot->plot_number).'*</a>';
