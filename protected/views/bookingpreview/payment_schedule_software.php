...
<head>
<meta charset="utf-8">
<title>Payment Schedule Software</title>
<style>
    .hide{
...
<div class="page">
  <div class="inner">
    ...
    <div class="row">
      <div class="title">PAYMENT SCHEDULE Software</div>
    ...
    <!-- Schedule table -->
    <div class="schedule">
        <?php $cop=0;if(@$booking->payment_schedule_software_json){?>
            <?php 
                // Decode JSON to PHP array
                $data = json_decode($booking->payment_schedule_software_json, true);
                $cop = $data['cop'];
...
<?php $extraFinal = 0; $sEx = 0;$tpEx = $booking->plot->total;$tpp = $booking->plot->total-$booking->plot->discount;?>


<?php if($booking->plot->is_corner == 1){?>
    <?php $tpp += $extraFinal += $this->Percentage($booking->plot->total,$booking->plot->is_corner_amount,false)?>
    <?php $tpEx += $this->Percentage($booking->plot->total,$booking->plot->is_corner_amount,false)?>
    
<?php } ?>
<?php if($booking->plot->is_road_facing == 1){?>
    <?php $tpp += $extraFinal += $this->Percentage($booking->plot->total,$booking->plot->is_road_facing_amount,false)?>
    <?php $tpEx += $this->Percentage($booking->plot->total,$booking->plot->is_road_facing_amount,false)?>
    
<?php } ?>
<?php if($booking->plot->is_park_facing == 1){?>
    <?php $tpp += $extraFinal += $this->Percentage($booking->plot->total,$booking->plot->is_park_facing_amount,false)?>
    <?php $tpEx += $this->Percentage($booking->plot->total,$booking->plot->is_park_facing_amount,false)?>
    
<?php } ?>
<?php if($booking->plot->is_west_open == 1){?>
    <?php $tpp += $extraFinal += $this->Percentage($booking->plot->total,$booking->plot->is_west_open_amount,false)?>
...
<?php } ?>
    ...
    <div class="costrow">
      ...
      <div class="right"><?php /*echo number_format($booking->plot->total)*/echo number_format($cop)?></div>
    </div>
    <?php
    $extras = [];
    ...
    <?php if (!empty($extras)) { ?>
        <div class="costrow">
            <div class="left">
                Extra Charges </br>(<?php echo implode(', ', $extras); ?>)
            </div>
            <div class="right">
                <?php echo number_format($extraFinal-$booking->plot->discount); ?>
            </div>
        </div>
    
        <div class="costrow">
          <div class="left">TOTAL COST OF PLOT</div>
    ...
    <div class="grid4 hide">
      ...
      <div class="bottom-grid">
        <div class="money">
          ...
          <div><b>Net Cost:</b></div><div>Rs:</div><div class="under"><?php echo 'Rs. '.number_format($cop)?></div>
        </div>
