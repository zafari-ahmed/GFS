        ...
        <div class="container content">
                ...
                foreach($modes as $index=>$mode):
                    ...
                    if($booking->customerpaymentSchedule){
                        ...
                        if($mode->mode == 'monthly'){
                            $mmode = $mode->mode;
                            $mode->mode = 'Monthly (<span class="calc"><s>36x'.(number_format((float)$oldModesData[$mmode]['amount']/36, 1, '.', '')).'/=</s>)</span>  (<span class="calc">36x'.(number_format((float)$mode->amount/36, 1, '.', '')).'/=)</span>';
            ...
            <?php if($booking->customerpaymentSchedule){?>
                <?php //if($plot->discount > 0) {?>
                    ...
                    Only cost of land in words: &nbsp;<span class="customSpan"><s><?php echo $this->getIndianCurrency($oldModesDataTotal)?></s>&nbsp;<?php echo $this->getIndianCurrency($newModesDataTotal)?></s></span><br/>
                <?php //} ?>
            <?php } else {?>
                <?php if($plot->discount > 0) {?>
