        ...
        <div class="container content">
            ...
            <b>PLOT ATTRIBUTES:</b> <span class="customSpan"><?php //echo number_format($extraCost).'/='?></span><br>
                <?php if($plot->customerPlots[0]->is_special != '' && $plot->customerPlots[0]->is_special != 0){?>
                    <span class="customSpan">
                        ...
                        <label>Payment Schedule&nbsp;&nbsp;&nbsp;<span class="label label-info" style="text-decoration: none;"> <?php echo @$plot->customerPlots[0]->special->name?> Custom PS</span></label>
                        <?php } else {?>
                ...
                foreach($modes as $index=>$mode):
                    ...
                    if($booking->customerpaymentSchedule){
                        ...
                        if($mode->mode == 'monthly'){
                            $mmode = $mode->mode;
                            $mode->mode = 'Monthly (<span class="calc">'.$booking->monthlyMonths.'x'.(number_format((float)$mode->amount/$booking->monthlyMonths, 1, '.', '')).'/=)</span>';
                        ...
                        if($mode->mode == 'possession'){
                            ...
                            // }
                            $modeAmount = round($mode->amount);
                            echo 'Last Payment:'.' <span>'.number_format($modeAmount).'/=</span>, ';    
                        } 
                    } else{
                        if($mode->mode == 'booking'){
                            echo ucfirst($mode->mode).':'.' <span>'.number_format(round($mode->amount)).'/=</span>, ';    
            ...
            <?php /*if($booking->customerpaymentSchedule){?>
                <?php //if($plot->discount > 0) {?>
                    Only Cost of land: <span class="customSpan"><?php  echo 'PKR '.number_format(@$oldModesDataTotal)?>/=</span>, &nbsp;
                    <?php $customDiscount = $oldModesDataTotal - $newModesDataTotal;?>
                    Discount <span class="customSpan"><?php echo number_format($customDiscount)?>/=</span>, &nbsp;
                    Discounted cost of land: &nbsp;<span class="customSpan"><?php echo ' PKR '.number_format($newModesDataTotal)?></span><br/>
                    Discounted cost of land in words: &nbsp;<span class="customSpan"><?php echo $this->getIndianCurrency($newModesDataTotal)?></s></span><br/>
                <?php //} ?>
            <?php } else {?>
                <?php if($plot->discount > 0) {?>
                ...
                <?php }?>
            <?php }*/?>
            Only Cost of land: <span class="customSpan"><?php echo ' PKR '.number_format($newModesDataTotal)?>/=</span><br/>
            Cost of land in words: &nbsp;<span class="customSpan"><?php echo $this->getIndianCurrency($newModesDataTotal)?></s></span><br/>
            <?php /*?>Extra land charges (if any): <span class="customSpan"><?php //echo @$extraCharge?> </span><?php */?>
            <br><br>
            I/we, the undersigned do hereby accept, confirm, affirm and agree to pay the cost of land and/or any other extra charges before taking possession of afore said plot in Gulshan e Bholari on the Terms and Conditions mentioned on the reverse.
            <br>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    Signature & Thumb of Allotee : <span></span> <div style="display: initial;margin-left: 28%;text-decoration: overline;">For Gulshan e Bholari<span></span></div>
                </div>
            </div>
            ...
            if($modes){
                foreach($modes as $index=>$mode):
                    ...
                    if($booking->customerpaymentSchedule){
                        ...
                        if($mode->mode == 'yearly'){
                            $mode->mode = 'Half Yearly (<span class="calc">6 x '.(number_format((float)$mode->amount/6, 1, '.', '')).'/=)</span>';
                            if($plot->discount > 0) {
                                $modeAmount = round($mode->amount);
                                //$discount = $plot->discount;
                                //$finalAmount = $modeAmount - $discount;
