                	...
                	<form role="form" method="POST" action="<?php echo Yii::app()->baseUrl?>/booking/update" enctype="multipart/form-data">
	                	<!-- <div class="col-lg-12"> -->
                            <div class="col-lg-12">
    	                		...
    	                		<div class="form-group col-lg-3" >
                                    <label>Plot #</label>
                                    <select  name="updated_plot_id" class="form-control select2">
                                    	<option value="">Please plot #</option>
                                    	<?php foreach($plots as $plot):?>
                                    		<option value="<?php echo $plot->id?>" <?php echo (@$booking->plot->id == $plot->id)?'selected':''?>><?php echo $plot->block_number?> - <?php echo $plot->plot_number?></option>
                                    	<?php endforeach;?>
                                    </select>
                            ...
                            <div class="col-lg-12">
                                ...
                                <div class="form-group col-lg-3">
                                    ...
                                    <!--</select>-->
                                    <input class="form-control" id="monthlyMonths" name="monthlyMonths" placeholder="Monthly Plan" value="<?php echo $booking->monthlyMonths?>">
                                    <!-- <p class="help-block">Example block-level help text here.</p> -->
                                </div>
                            ...
                            <div class="col-lg-12 <?php echo $class?>">
                                ...
                                <div class="form-group col-lg-3">
                                    ...
                                    <input class="form-control" name="agent_percentage" placeholder="Agent Commission" value="<?php echo @$booking->agent_percentage?>" autocomplete="off">
                                </div>
                                <?php } else {?>
                                    <input type="hidden" name="agent_percentage" value="<?php echo @$booking->agent_percentage?>">
                                <?php }?>
                                <div class="form-group col-lg-3">
                                ...
                                <div class="form-group col-lg-3">
                                    ...
                                    <input class="form-control calender" id="monthly_start_date" name="monthly_start_date" placeholder="Monthly Start Date" value="<?php echo $this->revertDateFormat($booking->monthly_start_date)?>">
                                </div>
	                    ...
	                    <div class="col-lg-12">
                            ...
                            <div class="form-group col-lg-6" style="padding-right: 0px;">
                                ...
                                <input class="form-control calender" id="dob" name="dob" placeholder="Date of Birth" value="<?php echo ($booking->customer->dob!='0000-00-00')?$this->revertDateFormat($booking->customer->dob):date('d-m-Y')?>">
                            </div>
                            <div class="form-group">
                                <label>CNIC</label>
                            ...
                            <div class="col-lg-12 form-group paymentModeBox" style="padding-left: 0px;" id="paymentModeBox_<?php echo $cpt->id?>">
                                ...
                                <div class="form-group col-lg-1">
                                    ...
                                    <select name="t_bank[]" id="t_bank" class="form-control">
                                        ...
                                        <option <?php echo ($cpt->bank=='Bank Makramah Limited (BML)')?'selected':''?> value="Bank Makramah Limited (BML)">Bank Makramah Limited (BML)</option>
                                ...
                                <div class="form-group col-lg-1">
                                    <label>Date</label>
                                    <input class="form-control calender" name="t_date[]" placeholder="Transaction Date" value="<?php echo $this->revertDateFormat(date('Y-m-d',strtotime($cpt->createdOn)))?>" autocomplete="off">
                                </div>
                                <div class="form-group col-lg-1">
                                    <label>Due Month</label>
                                    <?php
                                    $monthlyDate = @$cpt->monthlyDate;
                                    
                                    $beforeStar = strpos($monthlyDate, '*') !== false 
                                        ? trim(explode('*', $monthlyDate)[0]) 
                                        : null;
                                    
                                    // Explode the months if they exist
                                    $selectedMonths = [];
                                    if ($beforeStar) {
                                        // Convert "jan,feb,mar" to array ['jan', 'feb', 'mar']
                                        $selectedMonths = array_map('trim', explode(',', $beforeStar));
                                        // Optional: Capitalize first letter to match option values (Jan, Feb, etc.)
                                        $selectedMonths = array_map('ucfirst', $selectedMonths);
                                    }
                                    ?>
                                    
                                    <select name="t_monthlyDate[<?php echo $cpt->id?>][]" id="monthlyDate_<?php echo $cpt->id?>" class="form-control select2" multiple>
                                        <option value="Jan" <?php echo in_array('Jan', $selectedMonths) ? 'selected' : ''; ?>>Jan</option>
                                        <option value="Feb" <?php echo in_array('Feb', $selectedMonths) ? 'selected' : ''; ?>>Feb</option>
                                        <option value="Mar" <?php echo in_array('Mar', $selectedMonths) ? 'selected' : ''; ?>>Mar</option>
                                        <option value="Apr" <?php echo in_array('Apr', $selectedMonths) ? 'selected' : ''; ?>>Apr</option>
                                        <option value="May" <?php echo in_array('May', $selectedMonths) ? 'selected' : ''; ?>>May</option>
                                        <option value="Jun" <?php echo in_array('Jun', $selectedMonths) ? 'selected' : ''; ?>>Jun</option>
                                        <option value="Jul" <?php echo in_array('Jul', $selectedMonths) ? 'selected' : ''; ?>>Jul</option>
                                        <option value="Aug" <?php echo in_array('Aug', $selectedMonths) ? 'selected' : ''; ?>>Aug</option>
                                        <option value="Sept" <?php echo in_array('Sept', $selectedMonths) ? 'selected' : ''; ?>>Sept</option>
                                        <option value="Oct" <?php echo in_array('Oct', $selectedMonths) ? 'selected' : ''; ?>>Oct</option>
                                        <option value="Nov" <?php echo in_array('Nov', $selectedMonths) ? 'selected' : ''; ?>>Nov</option>
                                        <option value="Dec" <?php echo in_array('Dec', $selectedMonths) ? 'selected' : ''; ?>>Dec</option>
                                    </select>
                                </div>
                            </div>
                            <?php } endforeach;}?>
                            ...
                            <div class="col-lg-12 form-group paymentModeBox" style="padding-left: 0px;" id="paymentModeBox_<?php echo $cpt->id?>">
                                ...
                                <div class="col-lg-1" style="padding-left: 0px;">
                                    ...
                                    <select name="extra_mode[]"  class="form-control">
                                        <option <?php echo ($cpt->plot_payment_mode=='development')?'selected':''?> value="development">Development</option>
                                        <option <?php echo ($cpt->plot_payment_mode=='penalty')?'selected':''?> value="penalty">Penalty</option>
                                        <option <?php echo ($cpt->plot_payment_mode=='transfer_fee')?'selected':''?> value="transfer_fee">Transfer Fee</option>
                                        <option <?php echo ($cpt->plot_payment_mode=='lease_charges')?'selected':''?> value="lease_charges">Lease Charges</option>
                                        <option <?php echo ($cpt->plot_payment_mode=='others')?'selected':''?> value="others">Others</option>
                                        <option <?php echo ($cpt->plot_payment_mode=='road_facing')?'selected':''?> value="road_facing">Road Facing</option>
                                        <option <?php echo ($cpt->plot_payment_mode=='west_open')?'selected':''?> value="west_open">West Open</option>
                                        <option <?php echo ($cpt->plot_payment_mode=='corner')?'selected':''?> value="corner">Corner</option>
                                        <option <?php echo ($cpt->plot_payment_mode=='extra_land')?'selected':''?> value="extra_land">Extra Land</option>
                                        <option <?php echo ($cpt->plot_payment_mode=='park_facing')?'selected':''?> value="park_facing">Park Facing</option>
                                    </select>
                                ...
                                <div class="form-group col-lg-1">
                                    <label>Date</label>
                                    <input class="form-control calender" name="extra_t_date[]" placeholder="Transaction Date" value="<?php echo $this->revertDateFormat(date('Y-m-d',strtotime($cpt->createdOn)))?>" autocomplete="off">
                                </div>
                            </div>
                            <?php endforeach;}?>
